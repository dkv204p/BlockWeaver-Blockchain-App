...
class Blockchain:
    ...
    def load_chain_from_db(self):
        ...
        for block_data in blocks:
            block_id, timestamp, proof, previous_hash = block_data
            cursor.execute("SELECT sender, recipient, amount FROM transactions WHERE block_id = %s", (block_id,))
            transactions = cursor.fetchall()
            transactions = [{'sender': t[0], 'recipient': t[1], 'amount': t[2]} for t in transactions]
    ...
    def create_block(self, proof, previous_hash):
        ...
        for tx in self.pending_transactions:
            cursor.execute(
                "INSERT INTO transactions (block_id, sender, recipient, amount) VALUES (%s, %s, %s, %s)",
    ...
    def add_transaction(self, sender, recipient, amount):
        ...
        return self.get_last_block()['index'] + 1 if self.get_last_block() else 1
